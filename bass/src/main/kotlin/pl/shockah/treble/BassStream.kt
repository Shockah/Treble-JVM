...
open class BassStream(
		handle: Int,
		initialDevice: BassDevice,
		autoFree: Boolean
): BassChannel(handle, initialDevice, autoFree) {
	val bitrate: Float by channelAttribute(C.BASS_ATTRIB_BUFFER)

	var playbackBufferSize: Float by channelAttribute(C.BASS_ATTRIB_BUFFER)
	...
	override fun free(autoFree: Boolean) {
		...
		freed = true
		if (!autoFree) {
			if (!L.lib.BASS_StreamFree(handle))
				L.throwErrorException()
		}
